...
class DataCreator:
    ...
    @classmethod
    def check_semantic_color(self):
        _ROOT_FOLDER = '/home/roman/Documents/datasets/Apollo'
        _ROAD_FOLDER = 'road03'
        _label_folder = os.path.join(_ROOT_FOLDER, _ROAD_FOLDER, 'Label')
        _list_images = glob(_label_folder + '/**/*.png', recursive=True)
        _semantic_colors = [[70, 130, 180],[0, 0, 142],[255, 0, 0],[220, 20, 60],[119, 11, 32]]
        _semantic_color_flag = 0

        for i, img in enumerate(_list_images[500:]):
            img = cv2.imread(img, cv2.COLOR_BGR2RGB)
            img_one_hot = np.reshape(img, (img.shape[0]*img.shape[1], 3))
            semantic_mask = np.unique(img_one_hot,axis=0)
            if _semantic_colors[_semantic_color_flag] in img:
                plt.imsave('/home/roman/Desktop/{}.png'.format(_semantic_color_flag), img)
                if _semantic_color_flag == len(_semantic_colors):
                    break
                else:
                    _semantic_color_flag += 1
                # plt.imshow(img)
                # plt.show()
            print(str(i) + ' : ' + str(_semantic_color_flag))
    ...
    def change_images(self, color_image, label_image):
        ...
        contours, _ = cv2.findContours(label_image_gray_filtered, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
        contours_ = []
        ...
        #img1 = np.zeros((label_image_gray.shape[0], label_image_gray.shape[1], 3)).astype('uint8')
        #colors_ = [(227, 206, 166), (180, 120, 31), (138, 223, 178),
        #           (44, 160, 51), (153, 154, 251), (28, 26, 227),
        #           (214, 178, 202), (154, 61, 106), (153, 255, 255),
        #           (40, 89, 177), (111, 191, 253), (0, 127, 255)]
        ...
        for contourid in range(len(contours_)):
            ...
            #cv2.drawContours(img1, contours, contourid, colors_[contourid % len(colors_)], 1)
            #cv2.fillConvexPoly(img1, contours_[contourid], colors_[contourid % len(colors_)])
            ...
            if len([x for x, y in zip(label, [70, 130, 180]) if x == y]) == 3:
                ...
                cv2.fillConvexPoly(label_image_id, contours_[contourid], 2)
...
class DataCreatorSyntetic:
    ...
    def change_images(self, color_image, label_image):
        ...
        for contourid in range(len(contours_)):
            ...
            label = label_image[contours[contourid][0][0][1]][contours[contourid][0][0][0]]
            if len([x for x, y in zip(label, [255, 255, 255]) if x == y]) == 3:
                cv2.fillConvexPoly(label_image_id, contours_[contourid], 1)
...
def create_apollo_syntetic_dataset():
    fourcc = cv2.VideoWriter_fourcc(*'XVID')
    data_creator = DataCreatorSyntetic()
    ROOT_FOLDER = '/home/roman/Documents/datasets/ApolloSemantic'
    RESIZER_ROOT_FOLDER = '/home/roman/Documents/datasets/resized/ApolloSemantic'

    label_folder = os.path.join(ROOT_FOLDER, 'Segmentation')
    color_folder = os.path.join(ROOT_FOLDER, 'RGB')
    record_folders = ['18-00'] #['00-00', '06-00', '09-00', '13-00', '14-00', '17-00', '18-00']
    
    for record_folder in record_folders:

        record_color_folder = os.path.join(color_folder, record_folder)
        list_image = glob(record_color_folder+'/**/*.jpg', recursive=True)

        if not os.path.exists(os.path.join(RESIZER_ROOT_FOLDER, 'videos')):
                os.makedirs(os.path.join(RESIZER_ROOT_FOLDER, 'videos'))

        video_color = cv2.VideoWriter(os.path.join(RESIZER_ROOT_FOLDER, 'videos', '{}_color.avi'\
            .format(record_folder)),
                fourcc, 20.0, data_creator.shp)
        video_label = cv2.VideoWriter(os.path.join(RESIZER_ROOT_FOLDER, 'videos', '{}_label.avi'\
            .format(record_folder)),
                fourcc, 20.0, data_creator.shp)
        video_labelid = cv2.VideoWriter(os.path.join(RESIZER_ROOT_FOLDER, 'videos', '{}_labelid.avi'\
            .format(record_folder)),
                fourcc, 20.0, data_creator.shp)
        ...
        for counter, image in enumerate(list_image): # read images
            ...
            try:
                ...
                label_image = cv2.imread(image.replace('RGB', 'Segmentation')+'.png')
            ...
            out_color_path = os.path.join(RESIZER_ROOT_FOLDER, 'RGB', record_folder)
            out_label_path = os.path.join(RESIZER_ROOT_FOLDER, 'Segmentation', record_folder)
            ...
            with open('image_list_{}.txt'.format(record_folder), 'a', encoding='utf8') as f:
        ...
        print('{} complite'.format(record_folder))

def create_apollo_dataset():
    fourcc = cv2.VideoWriter_fourcc(*'XVID')
    data_creator = DataCreator()
    ROOT_FOLDER = '/home/roman/Documents/datasets/Apollo'
    RESIZER_ROOT_FOLDER = '/home/roman/Documents/datasets/resized/Apollo'
        ...
        for record_folder in record_folders: # read record folders0
            ...
            for camera_folder in camera_folders: # read camera folders
                ...
                for counter, image in enumerate(list_image): # read images
                    ...
                    out_color_path = os.path.join(RESIZER_ROOT_FOLDER, color_folder, camera_folder).replace(' ', '_')
                    out_label_path = os.path.join(RESIZER_ROOT_FOLDER, label_folder, camera_folder).replace(' ', '_')
                    ...
                    video_color.write(color_image_resized)
                    video_label.write(label_image_resized)
                    video_labelid.write(label_image_id_video*125)

                    with open('image_list_{}.txt'.format(road_folder), 'a', encoding='utf8') as f:
                        f.write(os.path.join(out_color_path,'{0:05}.jpg'.format(counter)) + ' ' +
                                os.path.join(out_label_path,'{0:05}.png'.format(counter)) + '\n')
...
if __name__ == '__main__':
    create_apollo_dataset()
    # create_apollo_syntetic_dataset()
    # DataCreator.check_semantic_color()
